# Prism 設計仕様書

## 1. コンセプト
光源(太陽光=白色ビーム)から出た光を、グリッド上に配置した部品で反射・分光・
ろ過・合成し、ゴールの結晶に「指定色の光」を届けるとクリア。限られた在庫の部品を
どう配置するかを考える制約パズル。

「単に繋ぐ」だけでなく **色を作る/選ぶ** という次元があるのが最大の特徴。
（例：白を分光して赤だけ通す／赤と青を別ルートで作って合成して紫にする）

## 2. コアメカニクス
- 盤面は正方グリッド(例 8×8)。各セルに部品を1つ置ける。
- 光は「線分の集合」。光源から発射し、部品/壁に当たるまで直進する。
- 光は **色(RGB)** を持つ。部品を通ると色や方向が変わる。
- ゴールが要求色を受け取ったらクリア。複数の光がゴールに入る場合は加法混色で合算。

## 3. 部品一覧(初期セット)
| 部品 | 挙動 |
|---|---|
| Source(光源) | 固定。指定方向へ白色 (1,1,1) のビームを発射 |
| Mirror(鏡) | 45°/135°。入射ビームを直角に反射(向きを変える) |
| Prism(プリズム) | 白色光を R/G/B の3本に分光し、それぞれ別方向へ出力 |
| Filter(色フィルタ) | 指定色成分だけ通す(入射色 × フィルタ色。例: 赤フィルタは R成分のみ通す) |
| Combiner(合成器) | 複数方向から入った光を加算し、1方向へ出力 |
| Wall(壁) | 光を吸収して停止させる。障害物・固定 |
| Goal(結晶) | 固定。受け取った色が目標色と誤差閾値内なら作動=クリア |

※ 部品には回転(0/90/180/270)を持たせ、入出力の辺を回転で決める。
※ 拡張候補: レンズ(集光/拡散)、分岐ミラー(ハーフミラー)、色を強めるブースター。

## 4. クリア条件
- 全ゴールが「作動」状態になったらステージクリア。
- 目標色との一致判定: `distance(received, target) <= tolerance`(例 tolerance = 0.15)。

## 5. 技術構成
- SwiftUI: タイトル / ステージ選択 / 設定。
- SpriteKit: 盤面描画・ビーム描画・部品ドラッグ。`SpriteView` でSwiftUIに埋め込む。
- 状態管理: `@Observable` な GameModel を SwiftUI と SKScene で共有。

## 6. データモデル(骨格)
```swift
enum ComponentKind: String, Codable {
    case source, mirror, prism, filter, combiner, wall, goal
}

enum Direction: Int, Codable { case up, right, down, left }

struct BeamColor: Codable {          // RGB 0...1
    var r, g, b: Float
    static let white = BeamColor(r: 1, g: 1, b: 1)
    func mixed(with o: BeamColor) -> BeamColor {  // 加法混色
        BeamColor(r: min(1, r+o.r), g: min(1, g+o.g), b: min(1, b+o.b))
    }
}

struct Position: Codable, Hashable { var x, y: Int }

struct PlacedComponent: Codable {
    var kind: ComponentKind
    var rotation: Int            // 0/90/180/270
    var color: BeamColor?        // filter/goal/source が使う目標色や発色
    var fixed: Bool              // true ならプレイヤーは動かせない(source/goal/wall等)
}

struct Level: Codable {
    var size: Int                       // グリッド一辺
    var grid: [Position: PlacedComponent]  // 固定配置(source/goal/wall/初期部品)
    var inventory: [ComponentKind: Int]    // プレイヤーが置ける部品の在庫
    var tolerance: Float
}

@Observable
final class GameModel {
    var level: Level
    var placements: [Position: PlacedComponent] = [:]  // プレイヤー配置
    var beams: [BeamSegment] = []                       // 再計算結果(描画用)
    var solved: Bool = false
    // 配置変更 → recomputeBeams() → クリア判定
}

struct BeamSegment {                 // 描画用の1本の線分
    var from: CGPoint
    var to: CGPoint
    var color: BeamColor
}
```

## 7. レイキャスト(光線計算)の流れ
盤面が変化したら `recomputeBeams()` を呼ぶ:
1. 全 Source を起点に、発射方向・白色でビームを1本ずつキューに入れる。
2. キューからビームを取り出し、現在セルから発射方向へ **1セルずつ前進**。
3. 次セルの部品を見て分岐:
   - なし → さらに前進(線分を延長)。盤外に出たら終了。
   - Wall / Goal → そのセルで停止。Goal なら「そのセルに入った色」を集計に加算。
   - Mirror → 反射方向を計算し、そのセルを新たな起点として続行。
   - Filter → 色を `received × filterColor` に更新して直進。
   - Prism → 白色なら R/G/B の3本に分割し、それぞれ別方向で新規ビームとしてキューに追加。
   - Combiner → 入射を一時保持。全入力が出そろったら加算して1本を出力。
4. 無限ループ対策: 「(セル, 進行方向, 色の丸め値)」の訪問済みセットを持ち、
   同一状態に再訪したら打ち切る(ミラーのループ対策)。
5. 各ゴールで受け取った色を合算し、目標色との距離でクリア判定。

## 8. ステージ定義(JSON例)
```json
{
  "size": 6,
  "tolerance": 0.15,
  "grid": {
    "0,2": { "kind": "source", "rotation": 90, "fixed": true },
    "5,2": { "kind": "goal", "rotation": 0, "fixed": true,
             "color": { "r": 1, "g": 0, "b": 0 } },
    "2,0": { "kind": "wall", "rotation": 0, "fixed": true }
  },
  "inventory": { "mirror": 2, "prism": 1, "filter": 1 }
}
```
※ 実装時は `[Position: ...]` を JSON化しやすいよう "x,y" 文字列キーで持つか、
  配列 `[{x,y,component}]` 形式にするか、どちらかに統一する(下記オープン事項)。

## 9. 実装マイルストーン(この順で1つずつ動く状態にする)
1. Xcode プロジェクト雛形 + `SpriteView` で SKScene を表示 + グリッド描画。
2. Source から1本のビームをレイキャストし、壁/盤外で停止するところまで描画。
3. Mirror(反射)を実装。ドラッグではなく、まずコードで固定配置してテスト。
4. Prism(白→R/G/B 分光)を実装。3本が別方向に出るのを確認。
5. Filter / Combiner / Goal 判定を実装。単純ステージを「解ける」状態にする。
6. ドラッグ&ドロップの配置UI + 在庫パレット + 回転操作。
7. ステージJSONローダ + クリア判定演出 + リセット/アンドゥ。
8. ステージ選択画面・チュートリアルステージ・エフェクト(発光・パーティクル)。

## 10. オープン事項(実装前に決めたい)
- グリッドサイズは固定(8×8)か、ステージ可変か。
- ビームは「セル中心を通る」前提か、任意角度を許すか(まずはセル中心・直角のみ推奨)。
- 合成器の入力タイミング(全入力待ちの実装方式)。
- ステージJSONのキー表現(辞書 vs 配列)。
- 色の丸め粒度(ループ検出の訪問済み判定に使う量子化ステップ)。

## 11. 実装済み要素一覧(現状)

### 部品(`ComponentKind`)
| 部品 | 挙動 | 備考 |
|---|---|---|
| Source(光源) | 指定方向へビームを発射。固定 | **発色は任意色に対応済み**(JSONの `color`。省略時は白)。ステージでは現状白のみ使用 |
| Mirror(鏡) | 「/」(rotation 0/180)・「\」(90/270)で直角反射 | 両面反射 |
| Prism(プリズム) | 入射光を R=直進 / G=左折 / B=右折 に分光 | 白以外の入射でも成分ごとに分解される |
| Filter(色フィルタ) | 透過色との **成分ごとの積**。直進 | フィルタ色はステージ定義で任意(現状は赤のみ使用) |
| Combiner(合成器) | 出力面以外から入った光を **加算合成** して1方向へ出力 | 全入力が出そろうまで盤面全体を再計算する不動点方式 |
| Splitter(スプリッタ/ハーフミラー) | 鏡と同じ向き規則で **半分反射・半分透過**(強度 0.5 倍ずつ) | スタンダードパック以降に登場 |
| Shifter(カラーシフタ/回転プリズム) | 色成分を **R→G→B→R に巡回シフト** して直進(赤→緑→青→赤。向きの影響なし) | ステージ22以降に登場 |
| Warp(ワープゲート) | 同じペア色の対ゲートへ光を転送し、**同じ進行方向のまま出る**。双方向・対が無ければ吸収 | 盤面固定の地形要素(`color` をペア識別に使用)。ステージ26以降に登場 |
| Wall(壁) | 光を吸収 | 固定 |
| Goal(結晶) | 受け取った光を加法混色で合算し、目標色と誤差判定 | 目標色は任意(半白・オレンジ等の中間色も使用中) |

### ルール・判定
- 光は **セル中心を通る直角レイキャスト**。盤面変化時のみ再計算。
- 色は RGB 各成分 0...1。合流は **加法混色(加算してクランプ)**。
- クリア判定は **最終的に各ゴールが受け取った色が目標色の許容誤差内か** のみ
  (解法は照合しない。別解・想定外の解法もクリア)。
- `tolerance` はステージごとに可変(現状は全ステージ 0.15)。
- グリッドサイズはステージ可変(現状 6〜8)。複数ゴール・複数光源に対応。
- 無限ループ対策: (セル, 方向, 1/16 量子化色) の訪問済み判定。
- 固定部品(`fixed: true`)はプレイヤーが移動・回転不可。

### ステージ構造・課金
- ステージは Codable JSON(`Levels/level_001.json` 形式の3桁連番)。フォルダに置くだけで一覧へ反映。
- パック: **無料 1〜10 / スタンダード 11〜50(IAP) / Extra 51〜100(IAP)**。
  - プロダクトID: `com.aryzae.aglaia.prism.pack.standard` / `com.aryzae.aglaia.prism.pack.extra`(非消耗型)
  - StoreKit 2(`PrismStore`)で購入・復元・`Transaction.currentEntitlements` による解放判定。
- 在庫(`inventory`)は部品種別+色ごとに個数制限。

### 操作・演出
- パレットの部品を **タップで選択**すると、盤面下に部品名と説明が表示される。
- 選択中に**盤面をドラッグすると半透明ゴースト+配置可否ハイライト**(緑=置ける/赤=不可)が
  指に追従し、離した位置に配置。タップ配置・パレットからのドラッグ&ドロップも可。
- 配置済み部品: タップで回転、ドラッグで移動(同ゴースト表示)、**盤外へドラッグで在庫へ回収**。
- アンドゥ / リセット。
- **クリア進捗の永続化**(`ProgressStore` / UserDefaults)。ステージ選択にクリア済みマークと
  パック別の「クリア n/m」を表示(全クリアで金色)。
- ビームは**進行方向へ流れる光の粒を常時アニメーション**させ、光源からの指向性がわかる。
- ゴール作動中のきらめき、クリア時のパーティクルバースト(SKEmitterNode)。

## 12. 難易度拡張の方針(レビュー反映済み)

### 強度(明るさ)の設計方針【決定】
- **許容誤差(tolerance)を絞る難化は不採用**。プレイヤーから見えないパラメータで難しくするのは分かりづらい。
- 光は「**色の種類**」と「**強度2段階(強 = 1.0 / 弱 = 0.5)**」だけで構成する。
  - ゴール目標色・フィルタ色・光源色の各成分は **0 / 0.5 / 1.0 のみ** を使う。
  - スプリッタ: 強 → 弱2本。**弱い光をさらに割る配置が解になるステージは作らない**
    (0.25 は原則登場させない。3段階目が必要になったら 0.25 を「微光」として追加検討)。
  - 弱 + 弱の合成 = 強。強度の帳尻合わせ自体をパズルにする。
  - `tolerance = 0.15` は実装上の判定余裕としてそのまま残す(0.5 刻みの色同士は距離 0.5 以上
    離れるため、誤クリア・誤判定は起きない)。
- **距離減衰【検討中・方向性は承認済み】**: 物理的な自然さがあるため、入れるなら
  「**一定セル数(例: 6セル)直進するごとに強度が1段階落ちる**」という離散形にする。
  連続減衰は2段階モデルと矛盾するので採用しない。ステージ単位のオプション
  (JSON に `attenuation` フラグ)として実装し、後半パックの限定ルールにする。

### A. コード追加なしで今すぐ可能(ステージデータだけで難化)【採用】
- **色付き光源**: 白以外の光源(実装済み・未使用)。分光しても欠けた成分しか出ない。
- **固定部品の初期配置**: 動かせない鏡・フィルタを盤面に置き、経路を強制する(実装済み・未使用)。
- **ダミー在庫**: 解に不要な部品を在庫に混ぜて探索空間を広げる。
- **グリッド拡大**: 9×9〜10×10。経路の組合せが爆発する。
- **ゴール数を増やす**: 3ゴール以上の同時成立。1本の白をどう配分するかのリソース管理になる。
- **両面鏡の両面活用**: 現行の鏡はすでに両面反射なので、**1枚の鏡の表と裏を2本のビームで
  同時に使わないと解けない配置**はデータだけで作れる。片面鏡(下記)導入後は
  「両面鏡ならでは」の個性としてさらに際立つ。

### B. 新部品(採用ロードマップ)
| 部品 | 挙動 | 投入先 |
|---|---|---|
| カラーシフタ(回転プリズム) | 成分を巡回シフト R→G→B→R | **実装済み**。ステージ22〜25で登場 |
| ワープゲート(ペア) | 対のセルへ光を転送(方向は維持) | **実装済み**。ステージ26〜50で登場(盤面固定の地形要素) |
| 一方通行ゲート(ダイオード) | 指定方向の光だけ通し、逆行は吸収 | Extra。逆流・自己干渉を防ぐ設計を強制 |
| 片面鏡 | 表は反射、裏から当たると吸収 | Extra。既存の鏡は「両面鏡」として明確に区別し、両者の使い分けを問う |
| 色スイッチ壁(ゲート) | 指定色の光をセンサーが受けている間だけ開く壁 | **Extraパック限定の目玉**。「まず鍵の色を作る→本命の経路を通す」段階構造。他要素と比べ異質さがあるため通常パックには入れない |

### B2. 保留(2段階強度モデルと整合する範囲で必要になったら)
- **減光板(強→弱)/ブースター(弱→強)**: 強度2段階モデルとは整合するが、
  スプリッタ・合成器と役割が重なるため当面見送り。減衰ルール導入時に再検討。
- **減算フィルタ**: 既存 Filter の色指定(例: マゼンタ = G除去)で表現できるため専用部品は不要。
- **十字分配器**: 1/3 強度が2段階モデルに反するため不採用。等分2方向ならスプリッタで足りる。
- **染色ミラー**: 部品の役割が複合的で説明コストが高い。保留。

### C. ルール・メタ要素の候補(未決定)
- **配置数制限 / パー(想定手数)**: 在庫より少ない個数でのクリアで星3などの評価。リプレイ性向上。
- **部品を置けない床(禁止セル)**: 光は通るが配置不可。経路の自由度だけを削る。
- **ヒントシステム**: 広告視聴 or 課金で1部品だけ正解位置を表示(マネタイズ補助)。
- **タイムアタック / デイリーステージ**: 同じ盤面資産で遊びの軸を増やす。

### 導入順
1. ~~**A(データのみ)** でスタンダードパック前半(13〜25)を量産~~ **完了**:
   13〜21 でデータのみ難化(固定部品13,18・色付き光源14,22〜24・両面活用16・ダミー在庫17,21,22・
   9×9盤面19〜21・部屋分断19・3〜4ゴール15,20)を使用
2. ~~**カラーシフタ**(色演算の拡張)~~ **完了**: ステージ22(入門)〜25(プリズム複合)で登場
3. ~~**ワープゲート**(経路の拡張)でスタンダード26〜50 の柱に~~ **完了**:
   26〜33 でワープ入門(通過・フィルタ併用・2ペア・双方向すれちがい・両面鏡連携)、
   34〜42 で10×10大盤面+複合、43〜50 で高難度(妨害用の固定シフタ・全色再合成・
   50番は全部品を使う総合ボス)。**スタンダードパック全40面が完成**
4. **一方通行ゲート・片面鏡・色スイッチ壁**(+必要なら距離減衰ルール)を Extra パックの柱に(52〜100 の量産と合わせて次の作業)

## 13. パクリ回避の確認ポイント
- 部品名・アイコン・BGM・UIデザイン・ステージ・ゲーム名はすべて独自に用意する。
- Electric Box の画像/音/文言/ステージを一切流用しない。
- 「連鎖変換パズル」というジャンル自体の流用は問題なし(ルール・アイデアは著作権対象外)。
  ※最終的な法的判断は専門家に確認すること。
